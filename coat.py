...
default_cfgs = {
    ...
    'coat_lite_small_mil': _cfg_coat_mil(
        url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-coat-weights/coat_lite_small-fea1d5a1.pth'
    ),
}
...
class GateAttentionLayer(nn.Module):
    def __init__(self, num_feas=1280, attention_size=64):
        ...
        self.feature_extractor_part2 = nn.Sequential(
            nn.Linear(self.num_feas, self.L),
            nn.ReLU(),
        )
    ...
    def forward(self, x):
        ...
        B,N,C = x.shape
        H = x.view(-1, C)   # (B*N)xC
        H = self.feature_extractor_part2(H)  # (B*N)xL

        A_V = self.attention_V(H)  # (B*N)xD
        A_U = self.attention_U(H)  # (B*N)xD
        A = self.attention_weights(A_V * A_U)  # element wise multiplication # (B*N)xK
        A = A.reshape(B, N, self.K)
        A = A.permute(0, 2, 1)  # BxKxN
        A = F.softmax(A, dim=2)  # softmax over N
        H = H.reshape(B,N,-1)
        M = A@H  # BxKxL
        M = M.reshape(-1, self.L)
        A = A.reshape(B, -1)
        return M, A


class CoaT_MIL(nn.Module):
    """ CoaT class. """

    def __init__(
            self, img_size=224, patch_size=16, in_chans=1, num_classes=2, embed_dims=(0, 0, 0, 0),
            serial_depths=(0, 0, 0, 0), parallel_depth=0, num_heads=0, mlp_ratios=(0, 0, 0, 0), qkv_bias=True,
            drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
            return_interm_layers=False, out_features=None, crpe_window=None,
            final_drop=0.5, **kwargs):
        super().__init__()
        crpe_window = crpe_window or {3: 2, 5: 3, 7: 3}
        self.return_interm_layers = return_interm_layers
        self.out_features = out_features
        ...
        self.attn = GateAttentionLayer(num_feas=512, attention_size=64)
    ...
    def forward_features(self, x):
        batch_size, num_instance, num_channel, width, height = x.shape
        # x = x.view(batch_size, -1, width, height).contiguous()  # (batch_size, num_instance, width, height)

        output_slice_list = []
        for i in range(num_instance):
            x_slice_ = x[:, i, :, :, :]  # (batch_size, num_channel, width, height)
            output_slice_ = self.embedding_net(x_slice_)  # (batch_size, num_feas)  embedding_net
            output_slice_ = output_slice_.view(batch_size, 1, -1)
            output_slice_list.append(output_slice_)
        x = torch.cat(output_slice_list, 1).contiguous()
        x, A0_ = self.attn(x)
        return x,A0_
    ...
    def forward(self, x):
        ...
        output_slice_list = []
        for i in range(num_instance):
            x_slice_ = x[:, i:i + 1, :, :]  # (batch_size, 1, width, height)
            output_slice_ = self.embedding_net(x_slice_)  # (batch_size, num_feas)  embedding_net
            output_slice_ = output_slice_.view(batch_size, 1, -1)
            output_slice_list.append(output_slice_)
        x = torch.cat(output_slice_list, 1).contiguous()
...
def _create_coat_mil(variant, pretrained=False, default_cfg=None, **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')

    model = build_model_with_cfg(
        CoaT_MIL, variant, pretrained,
        default_cfg=default_cfgs[variant],
        pretrained_filter_fn=checkpoint_filter_fn,
        **kwargs)
    return model
...
@register_model
def coat_lite_small_mil(pretrained=False, **kwargs):
    model_cfg = dict(
        patch_size=4, embed_dims=[64, 128, 320, 512], serial_depths=[3, 4, 6, 3], parallel_depth=0,
        num_heads=8, mlp_ratios=[8, 8, 4, 4], **kwargs)
    model = _create_coat_mil('coat_lite_small_mil', pretrained=pretrained, **model_cfg)
    return model
...
if __name__ == '__main__':
    model_cfg = dict(
        patch_size=4, embed_dims=[64, 128, 320, 512], serial_depths=[3, 4, 6, 3], parallel_depth=0,
        num_heads=8, mlp_ratios=[8, 8, 4, 4], num_classes=2, in_chans=1, final_drop=0.5)
    model = _create_coat_mil('coat_lite_small_mil', pretrained=False, **model_cfg)
    x = torch.zeros(4,7,1,224,224)
    print(x.shape)
    out_ = model.forward_features(x)
    print(out_.shape)